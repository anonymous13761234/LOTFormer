...
class ModelForSC(nn.Module):
    def __init__(self, config):
        ...
        # optional, but recommended: prefer bf16 if available
    ...
    def forward(self, input_ids_0, mask_0, label):
        # backbone in AMP
        with torch.amp.autocast('cuda', dtype=self.amp_dtype, enabled=self.enable_amp):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            seq_scores = self.seq_classifer(token_out.float())  # logits in fp32
            seq_loss   = nn.CrossEntropyLoss(reduction="none")(seq_scores, label)
            seq_accu   = (seq_scores.argmax(dim=-1) == label).float()

        return {"loss": seq_loss, "accu": seq_accu, "logits": seq_scores}
...
class ModelForSCDual(nn.Module):
    ...
    def forward(self, input_ids_0, input_ids_1, mask_0, mask_1, label):
        ...
        # head + loss in fp32
        with torch.amp.autocast('cuda', enabled=False):
            seq_scores = self.seq_classifer(token_out_0.float(), token_out_1.float())
            seq_loss   = nn.CrossEntropyLoss(reduction="none")(seq_scores, label)
            seq_accu   = (seq_scores.argmax(dim=-1) == label).float()
