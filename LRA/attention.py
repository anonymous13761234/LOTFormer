...
def attn_selector(attn_type, config, W_q=None, W_k=None, W_v=None):
    ...
    if attn_type.startswith("softmax"):
        ...
        from attention_informer import ProbAttention
        ...
        from attention_bigbird import BigBirdAttention
        ...
        from attention_skyformer import Skyformer
        attn = Skyformer(config)
    elif attn_type.startswith("sinkhorn_linear"):
        from linear_sinkformer import LinearSinkAttention
        attn = LinearSinkAttention(config)
    return attn
...
class Attention(nn.Module):
    ...
    def forward(self, X, mask):
        # get the device where the attention module lives
        device = next(self.attn.parameters()).device
    
        # handle longformer/reformer branch (single-tensor attention)
        if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer"):
            X = X.to(device, non_blocking=True)
            if mask is not None:
                mask = mask.to(device, non_blocking=True)
            # autocast disabled here by choice; leave as-is
            with torch.amp.autocast(device_type="cuda", enabled=False):
                attn_out = self.attn(X.float(), None if mask is None else mask.float())
        else:
            # standard MHA path: make sure inputs are on the same device as self.attn
            X = X.to(device, non_blocking=True)
            if mask is not None:
                mask = mask.to(device, non_blocking=True)
            ...
            # (defensive) ensure Q/K/V are also on the same device
            ...
            with torch.amp.autocast(device_type="cuda", enabled=False):
                if self.grad_checkpointing:
                    attn_out = checkpoint(self.attn,
                                          Q.float(), K.float(), V.float(),
                                          None if mask is None else mask.float())
                else:
