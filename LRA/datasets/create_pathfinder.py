...
for diff_level in ["curv_baseline", "curv_contour_length_9", "curv_contour_length_14"]:
    ...
    with open(f"./lra-{subdir}-{diff_level}.train.pickle", "wb") as f:
        pickle.dump(train, f)
    with open(f"./lra-{subdir}-{diff_level}.dev.pickle", "wb") as f:
        pickle.dump(dev, f)
    with open(f"./lra-{subdir}-{diff_level}.test.pickle", "wb") as f:
        pickle.dump(test, f)
