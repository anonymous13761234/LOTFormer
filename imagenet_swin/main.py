...
from utils import load_checkpoint, save_checkpoint, save_checkpoint_new, get_grad_norm, auto_resume_helper, reduce_tensor, load_pretrained
...
def main():
    ...
    for epoch in range(config.TRAIN.START_EPOCH, total_epochs):
        ...
        if dist.get_rank() == 0 and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs)):
            save_checkpoint_new(config, epoch + 1, model_without_ddp, max(max_accuracy, acc1), optimizer, lr_scheduler, logger)

        if dist.get_rank() == 0 and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs)) and acc1 >= max_accuracy:
            save_checkpoint_new(config, epoch + 1, model_without_ddp, max(max_accuracy, acc1), optimizer, lr_scheduler, logger, name='max_acc')
        max_accuracy = max(max_accuracy, acc1)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, logger, total_epochs):
    ...
    for idx, (samples, targets) in tqdm(enumerate(data_loader), disable=(not is_main)):
