...
class SinkhornDistance(nn.Module):
    ...
    def forward(self, c, mu=None, nu=None):
        ...
        for i in range(self.max_iter):
            if i % 2 == 0:
                u1 = u.clone()  # useful to check the update
                M = self.M(C, u, v)
...
class LotLinearAttention(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    ...
    def __init__(self, dim, window_size, num_heads, num_refs, sink_eps, sink_max_iter, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
    ...
    def forward(self, x, H, W, mask=None):
        ...
        D = self.head_dim
        BH = B * self.num_heads
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads * D).permute(2, 0, 1, 3)
        Q, K, V = qkv.unbind(0)
        ...
        Qb = Q.reshape(B, N, self.num_heads, D).permute(0, 2, 1, 3).reshape(BH, N, D).to(torch.float32) * self.scale
        Kb = K.reshape(B, N, self.num_heads, D).permute(0, 2, 1, 3).reshape(BH, N, D).to(torch.float32) * self.scale
        Vb = V.reshape(B, N, self.num_heads, D).permute(0, 2, 1, 3).reshape(BH, N, D).to(torch.float32) 

        z_head = torch.softmax(self.z_logits / 0.5, dim=-1)           # (H,R)
        z = z_head.repeat_interleave(BH // self.num_heads, dim=0)             # (BH,R)

        # Compute attention scores with gradient clipping
        S_qz = torch.matmul(Qb, Z.transpose(-1, -2))  # (BH,N,R)

        S_zk = torch.matmul(Kb, Z.transpose(-1, -2))  # (BH,N,R)
        S_zk = S_zk.transpose(-1, -2) # (BH,R,N)

        P_QZ = self.sink(S_qz, nu=z)[0]  # (BH,N,R)
        P_ZK = self.sink(S_zk, mu=z)[0]  # (BH,R,N)
        
        # inv_sqrt_z = torch.full((BH, R), R**-0.5, device=device, dtype=torch.float32)
        z_zero_mask = z == 0
        inv_sqrt_z = z.sqrt().reciprocal() # (BH,R)
        ...
        x = N * torch.matmul(Phi_Q, KV).reshape(B, self.num_heads, N, D).permute(0, 2, 1, 3).reshape(B, N, self.num_heads * D)  # (BH,N,D)

        H = W = int(N ** 0.5)
        v = Vb.reshape(BH, H, W, -1).permute(0, 3, 1, 2)
        before_shape = v.shape
        v_post = self.dwc(v)
        after_shape = v_post.shape
        v = v_post.reshape(B, self.num_heads * D, N).permute(0, 2, 1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, num_refs, sink_eps, sink_max_iter, window_size=7, shift_size=0,
        ...
        if attn_type == 'L':
            self.attn = LotLinearAttention(
                dim, window_size=to_2tuple(self.window_size), num_heads=num_heads, num_refs=num_refs, sink_eps=sink_eps, 
                sink_max_iter=sink_max_iter, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, 
                proj_drop=drop, alpha=alpha, kernel_size=kernel_size)
        else:
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, H, W, mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 num_refs, sink_eps, sink_max_iter,
...
class LotSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 num_refs=32, sink_eps=1.0, sink_max_iter=5,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False,
                 alpha=4, kernel_size=5, attn_type='LLLL', **kwargs):
        super().__init__()
